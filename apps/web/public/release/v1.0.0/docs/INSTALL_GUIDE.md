...
软件首次运行后会在本机开启 3 天试用，不需要登录或联网。

试用结束后：

1. 在软件授权页面复制机器码并发送给管理员。
2. 管理员根据机器码生成激活码或 `license.mrx` 授权文件。
3. 在软件中输入激活码，或导入 `license.mrx`，完成本地校验。

机器码、激活码和授权文件只用于离线授权校验。软件不会上传用户处理的图片、PDF、Word、Excel、音频或视频文件。
