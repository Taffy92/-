...
- [x] Node 20 / pnpm 9.15.4 下 `npm run verify:release` 完整通过（2026-08-09，11/11 阶段）。
- [x] 门禁固定执行完整测试、隐私、网络、Cargo check/test 和真实 Office 转换。
- [x] 门禁构建 EdgeOne 产物并核对 ZIP、MSI、版本、SHA256、分片大小和分片哈希。
- [x] 门禁拒绝 Git 或公开产物中的私钥、客户记录和 `.mrx`。
- [x] 门禁拒绝关键转换测试中的 `skip` / `todo`。
- [x] 门禁本身不部署、不删除正式产物、不修改版本号。
- [ ] EdgeOne 授权后台登录限流生产黑盒验证完成。
- [ ] Windows 10 / 11 干净虚拟机验收完成。
