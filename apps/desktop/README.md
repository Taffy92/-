...
Windows 安装包使用 Tauri `webviewInstallMode.type = "offlineInstaller"`，会把 WebView2 Runtime 离线安装器嵌入 MSI。这样在没有网络的 Windows 10/11 x64 电脑上也可以完成安装并使用核心功能。
...
下载页仅提供 ZIP 安装包；下载后请先核对 SHA256，再解压 ZIP 内的 MSI 安装包。
